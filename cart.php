...
function renderCart(items) {
  ...
  if (!items || items.length === 0) {
    ...
    subtotalEl.setAttribute('data-price', '0');
    subtotalEl.textContent = '£0.00';
    totalEl.setAttribute('data-price', '0');
  ...
  const cartHtml = items.map(item => {
    ...
    let variant = '';
    if (item.material_name) variant += `Material: ${item.material_name}`;
    if (item.color) variant += (variant ? ', ' : '') + `Color: ${item.color}`;
    if (item.finish) variant += (variant ? ', ' : '') + `Finish: ${item.finish}`;
    if (item.size) variant += (variant ? ', ' : '') + `Size: ${item.size}`;

    return `
      <div class="cart-item">
        <div class="thumb">
          <img src="${item.image || 'assets/images/placeholder.jpg'}" alt="${item.product_name}" onerror="this.src='assets/images/placeholder.jpg'">
        </div>
        <div class="meta">
          ...
          <div class="row-bottom">
            <div class="qty-control">
              <button onclick="updateQuantity('${item.cart_item_id}', ${quantity - 1})">-</button>
              <input type="number" value="${quantity}" min="1" onchange="updateQuantity('${item.cart_item_id}', this.value)">
              <button onclick="updateQuantity('${item.cart_item_id}', ${quantity + 1})">+</button>
            </div>
            <div class="price-line">£${itemTotal.toFixed(2)}</div>
            <button class="remove-from-cart" onclick="removeItem('${item.cart_item_id}')">Remove</button>
          </div>
        </div>
      </div>
    `;
  ...
  subtotalEl.setAttribute('data-price', subtotal.toString());
  subtotalEl.textContent = `£${subtotal.toFixed(2)}`;
  totalEl.setAttribute('data-price', subtotal.toString());
