...
// Define a fixed, ordered list for genders
$all_genders = ['Female', 'Male', 'Unisex']; 
...
// Handle gender filter. This assumes gender is a category.
if (!empty($selected_gender)) {
    $selected_gender_lower = strtolower($selected_gender);
    if ($selected_gender_lower === 'male' || $selected_gender_lower === 'female') {
        // A product is considered 'male' if it's in 'Male' or 'Unisex' categories
        $where[] = "p.id IN (SELECT product_id FROM product_categories pc_gender JOIN categories c_gender ON pc_gender.category_id = c_gender.id WHERE LOWER(c_gender.name) IN (?, 'unisex'))";
        $params[] = $selected_gender_lower;
    } else {
        // Handles 'unisex' or any other gender category directly
        $where[] = "p.id IN (SELECT product_id FROM product_categories pc_gender JOIN categories c_gender ON pc_gender.category_id = c_gender.id WHERE LOWER(c_gender.name) = ?)";
        $params[] = $selected_gender_lower;
    }
}
...
// --- DEBUGGING ---
echo "<pre><strong>DEBUGGING OUTPUT:</strong><br>";
echo "<strong>Applied Filters:</strong><br>";
echo "Category: " . htmlspecialchars($category_filter) . "<br>";
echo "Gender: " . htmlspecialchars($gender_filter) . "<br>";
echo "Price: " . htmlspecialchars($price_filter) . "<br>";
echo "<hr>";
echo "<strong>Generated SQL Query:</strong><br>" . htmlspecialchars($sql) . "<br>";
echo "<hr>";
